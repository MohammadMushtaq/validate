...
public class VendorServiceImpl implements VendorService {
	...
	public boolean validateAndSave(VendorEntity entity) {
		...
		if (entity.getName() != null && entity.getName().length() > 4 && entity.getName().length() < 30) {
			valid = true;
		} else {
		...
		if (entity.getEmail() != null && entity.getEmail().isEmpty() && entity.getEmail().contains("@")
				|| entity.getEmail().contains(".com") || entity.getEmail().contains(".org")) {
			valid = true;
		} else {
...
//		}
	...
	public boolean validateLoginAndPassword(String loginName, String password) {
		boolean valid = false;
		if (loginName != null && loginName.length() >= 8 && loginName.length() < 13) {
			valid = true;
		...
		if (valid) {
			boolean val = dao.validateAndLogin(loginName, password);
			System.out.println("login is matching " + val);
			return val;
		}
	...
	@Override
	public void validateEmailAndChangePassword(String email, String newPassword) {
		boolean valid = true;
		...
		if (valid) {
			boolean val = dao.validateByEmail(email);
			System.out.println(" found email " + val);
			if (valid) {
				dao.updatePasswordByEmail(email, newPassword);
				System.out.println("updated password by email " + email);
			}
		} else {
